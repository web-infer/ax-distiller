...
const (
	ROLE_SYNTHETIC_LIST   = "SYNTHETIC_LIST"
	ROLE_SYNTHETIC_OBJECT = "SYNTHETIC_OBJECT"
)

func newSyntheticList() *cdp.AXNodeWithRelatives {
	return &cdp.AXNodeWithRelatives{
		Underlying: cdp.AXNode{
			Role: cdp.Value[string]{Value: ROLE_SYNTHETIC_LIST},
		},
	}
}

func newSyntheticObject() *cdp.AXNodeWithRelatives {
	return &cdp.AXNodeWithRelatives{
		Underlying: cdp.AXNode{
			Role: cdp.Value[string]{Value: ROLE_SYNTHETIC_OBJECT},
		},
	}
}
...
outer:
	...
	inner:
		for cursor := start; ; cursor = cursor.NextSibling {
			...
			patternCursor = cursor
			for range length {
				buff = binary.BigEndian.AppendUint64(buff, patternCursor.Hash)
				patternCursor = patternCursor.NextSibling
			}
...
func convertToStructure(start *cdp.AXNodeWithRelatives) (ret *StructureInstance) {
	...
	*/
	...
	for cur := start; cur != nil; cur = cur.NextSibling {
		...
		hashBuff = append(hashBuff, []byte(cur.Underlying.Role.Value)...)
