...
func NewPersistent(logger *slog.Logger, dbDriver, stdbDriver *sql.DB) *Persistent {
	return &Persistent{
		logger:     logger,
		dbDriver:   dbDriver,
...
func (p *Persistent) HandleEvent(ctx context.Context, e axstream.Event) (err error) {
	...
	case axstream.EVENT_PATCH:
		for _, updated := range e.Updated {
			...
			err = p.pushDB(ctx, st)
			if err != nil {
				err = fmt.Errorf("push db (%s): %w", updated.Underlying.NodeID, err)
				return
			}
		}
	}
...
// annotated structure
func (p *Persistent) initStructTree(axroot *cdp.AXNodeWithRelatives) (out *StructureInstance) {
...
func (p *Persistent) makeStructTree(axnode *cdp.AXNodeWithRelatives) (out *StructureInstance) {
...
func (p *Persistent) makeSynthetics(st *StructureInstance) {
	p.makeSyntheticsInner(st)
	// p.updateSyntheticIDs(st)
}

func (p *Persistent) makeSyntheticsInner(st *StructureInstance) {
...
func (p *Persistent) pushDB(ctx context.Context, subtree *StructureInstance) (err error) {
...
func (op *pushDBStateOp) Do(ctx context.Context, root *StructureInstance) (err error) {
	...
	if err != nil {
		err = fmt.Errorf("ephemeral: %w", err)
		return
	}
	err = op.pushStructures(ctx, root)
	if err != nil {
		err = fmt.Errorf("persistent: %w", err)
		return
	}
	return
}
...
func (op *pushDBStateOp) pushInstances(ctx context.Context, root *StructureInstance) (err error) {
	...
	if err != nil {
		err = fmt.Errorf("push root (%d): %w", st.ID, err)
		return
	}

	// we update its ancestors
	err = op.updateAncestorSHash(ctx, st)
	if err != nil {
		err = fmt.Errorf("update ancestors (%d): %w", st.ID, err)
		return
	}
...
func (op *pushDBStateOp) pushStructures(ctx context.Context, node *StructureInstance) (err error) {
	...
	if err != nil {
		err = fmt.Errorf("structure: %w", err)
		return
	}
	...
	for child := node.FirstChild; child != nil; child = child.NextSibling {
		...
		if err != nil {
			err = fmt.Errorf("child (%s): %w", child.Underlying.Underlying.NodeID, err)
			return
		}
	}
...
// updateAncestors updates the shash of all ancestors of the given root in the
// stdb
func (op *pushDBStateOp) updateAncestorSHash(ctx context.Context, root stdb.StructureInstance) (err error) {
	current := root
