...
func NewBundledRequests[ID comparable](ctx context.Context, targets []ID) *BundledRequests[ID] {
...
waitsub:
	for {
		...
		case <-s.req:
			s.subscribed++
			// slog.Debug("[bundle] request update", "subscribed", s.subscribed, "total", len(s.targets))
	...
	// slog.Debug("[bundle] start child wait", "total", len(s.children))
	for _, childRequest := range s.children {
		select {
		case <-s.ctx.Done():
			s.done <- false
			return
		...
		// slog.Debug("[bundle] pass child wait", "progress", i+1, "total", len(s.children))
	}

	// slog.Debug("[bundle] send done")
	s.done <- true
}
